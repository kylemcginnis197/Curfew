...
// claudeProvider builds a Claude provider bound to a specific config dir.
func claudeProvider(name, configDir string) Provider {
	return Provider{
		Name:          name,
...
// CodexPreset anchors an OpenAI Codex window. Window length and log location are
// best-effort defaults the user can tune; if the logs can't be read, state
// detection falls back to "assume no active window".
